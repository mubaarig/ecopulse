...
export function SupplyChainMap({ ticker }: SupplyChainMapProps) {
  ...
  const totalEmissions =
    supplyChainData?.reduce((sum, item) => sum + item.emission, 0) || 0;
  ...
  return (
    <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
      ...
      <div className="space-y-4">
        ...
        <div className="p-3 rounded-lg border border-amber-200 bg-amber-50">
          <div className="flex items-center space-x-2 mb-1">
            ...
            <span className="text-sm font-medium text-amber-800">
              {highRiskCountries} High-Risk Region
              {highRiskCountries !== 1 ? "s" : ""}
            </span>
          </div>
          <p className="text-xs text-amber-700">
            Consider diversifying suppliers in high-risk regions to mitigate
        ...
        <GlobeVisualization supplyChainData={supplyChainData || []} />
      </div>
    </div>
  );
}

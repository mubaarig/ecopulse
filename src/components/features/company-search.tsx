...
const filterOptions = ['All', 'Technology', 'Energy', 'Automotive', 'Financials', 'Consumer Staples'];
...
export function CompanySearch() {
  ...
  const filteredCompanies = useMemo(() => {
    ...
    if (activeFilter !== 'All') {
      results = results.filter((company) => company.industry === activeFilter);
    }
    ...
    return results
      .filter((company) => {
        const haystack = `${company.name} ${company.ticker} ${company.industry}`.toLowerCase();
          ...
          <ul className="space-y-3">
            {filteredCompanies.map((company) => (
              ...
              >
                ...
                <Button variant="ghost" className="gap-2 text-sm font-semibold text-emerald-600 hover:bg-emerald-50">
                  Open profile
                  <ArrowRight className="h-4 w-4" />
                </Button>
              </li>
              ...
              <div>
                ...
                <p className="mt-1">
                  ...
                  <span className="font-medium text-emerald-800">
                    "Scope 3 laggards in automotive suppliers with new CDP filings"
                  </span>
                </p>
              </div>
            </div>
